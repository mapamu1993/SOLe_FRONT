...
const Navbar = () => {
  ...
  // --- 2. AQUÍ ESTABA EL ERROR: AÑADIMOS 'user' ---
  ...
  const navLinks = [
    { name: "Home", path: "/" },
    { name: "Nosotros", path: "/about" }, // Asegúrate de que esta ruta exista o cámbiala por la correcta
    ...
    <>
      <nav className={navClasses}>
        ...
        <div className="flex items-center gap-3">
          ...
          {/* --- 3. LÓGICA DE PERFIL / LOGIN CORREGIDA --- */}
          <Link to={isAuthenticated ? "/profile" : "/login"}>
            {isAuthenticated ? (
              // USUARIO LOGUEADO: Foto o Icono
                  ...
                  <img
                    src={
                      item.product.image?.startsWith("http")
                        ? item.product.image
                        : `${IMAGE_URL}/uploads/products/${item.product.image}`
                    }
