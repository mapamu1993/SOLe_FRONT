...
interface ProductFormDesignProps {
  ...
  existingImageUrl?: string | null; // <--- NUEVA PROP
}
...
export const ProductFormDesign = ({
  ...
  onFileChange,
  currentFile,
  existingImageUrl, // <--- RECIBIR PROP
}: ProductFormDesignProps) => {
  
  // LOGICA PREVIEW: Si hay archivo nuevo, usa ese. Si no, usa la URL existente.
        ...
        <div className="rounded-[2.5rem] bg-white p-8 md:p-12 shadow-2xl border border-[#333D29]/5 relative overflow-hidden">
          ...
          <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#EBECE2] pb-6 relative z-10">
            <div>
              ...
              <p className="mt-2 text-sm text-[#656D4A] font-medium">Gestiona el inventario de tu tienda</p>
            </div>
            <Link to="/tienda">
          ...
          <form onSubmit={onSubmit} className="space-y-8 relative z-10">
            ...
            <div className="space-y-3">
              <Label htmlFor="name" className="text-[#333D29] font-bold uppercase tracking-wider text-xs ml-1">Nombre del Producto</Label>
              <Input 
                id="name" 
              ...
              />
              {errors.name && <span className="text-xs font-bold text-red-500 ml-1">{errors.name.message}</span>}
            </div>

            <div className="space-y-3">
              <Label htmlFor="description" className="text-[#333D29] font-bold uppercase tracking-wider text-xs ml-1">Descripción</Label>
              <textarea
                id="description"
                rows={4}
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="space-y-3">
                <Label htmlFor="price" className="text-[#333D29] font-bold uppercase tracking-wider text-xs ml-1">Precio (€)</Label>
                <div className="relative">
                  ...
                  />
                  <IconCurrencyEuro className="absolute left-3 top-1/2 -translate-y-1/2 text-[#B6AD90]" size={20} />
                </div>
                {errors.price && <span className="text-xs font-bold text-red-500 ml-1">{errors.price.message}</span>}
              </div>

              <div className="space-y-3">
                <Label htmlFor="stock" className="text-[#333D29] font-bold uppercase tracking-wider text-xs ml-1">Stock Disponible</Label>
                <div className="relative">
                  <Input
                    id="stock"
                    type="number"
                    ...
                    className="bg-[#F5F5F0] border-transparent focus:border-[#B6AD90] focus:ring-0 text-[#333D29] placeholder:text-[#333D29]/40 h-14 rounded-xl text-lg font-medium pl-10 shadow-none"
                  />
                  <IconPackage className="absolute left-3 top-1/2 -translate-y-1/2 text-[#B6AD90]" size={20} />
                </div>
                {errors.stock && <span className="text-xs font-bold text-red-500 ml-1">{errors.stock.message}</span>}
              </div>
            </div>

            <div className="space-y-3">
              <Label htmlFor="category" className="text-[#333D29] font-bold uppercase tracking-wider text-xs ml-1">Categoría</Label>
              <div className="relative">
                <Input
                  id="category"
                  ...
                  className="bg-[#F5F5F0] border-transparent focus:border-[#B6AD90] focus:ring-0 text-[#333D29] placeholder:text-[#333D29]/40 h-14 rounded-xl text-lg font-medium pl-10 shadow-none"
                />
                <IconCategory className="absolute left-3 top-1/2 -translate-y-1/2 text-[#B6AD90]" size={20} />
              </div>
              {errors.category && <span className="text-xs font-bold text-red-500 ml-1">{errors.category.message}</span>}
            </div>

            <div className="space-y-3">
              <Label className="text-[#333D29] font-bold uppercase tracking-wider text-xs ml-1">Imagen del Producto</Label>
                ...
                <div className="relative h-64 w-full flex flex-col items-center justify-center text-[#656D4A]">
                  {previewUrl ? (
                    ...
                    <div className="flex flex-col items-center gap-2 p-6 text-center">
                      ...
                      <p className="font-bold text-sm">Sube la foto del artículo</p>
                      <p className="text-xs opacity-70">JPG, PNG preferiblemente fondo transparente</p>
                    </div>
                  )}
                </div>
